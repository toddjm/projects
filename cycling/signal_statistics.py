...
def lift(predictor, target, pos_thresh=0.0, neg_thresh=0.0, plot=True):
    """
    Construct lift curves and summary statistics for
    ...
    data = np.column_stack((predictor, target))
    ...
    # Compute positive and negative lift.
    lift_pos = np.where(data[:, 1] >
                        pos_thresh, 1.0, 0.0).cumsum()
    lift_neg = np.where(data[:, 1][::-1] <
                        neg_thresh, -1.0, 0.0).cumsum()

    # mean lift = (total # of targets in class buy or sell) / nobv
    # output [column]:
    # [0] percent of ordered observations
    # [1] raw lift
    # [2] ratio of lift to mean lift for the percentile
    # [3] number of samples in the percentile
    ...
    raw_lift_pos = np.insert(
        (np.array([lift_pos[i] / float(i) for i in range(1, nobv)])),
        0, 0.0)
    raw_lift_neg = np.insert(
    ...
    lift_neg -= np.array([lift_neg[-1] / float(nobv)] * nobv).cumsum()

    if plot:
        plot_lift(lift_pos, data[:, 0], lift_neg, data[:, 0][::-1])
    return
...
def plot_lift(y_1, y_2, y_3, y_4):
    ...
    p_1.set_ylabel('normalized lift - positive', color='b')
    ...
    p_3 = p_1.twinx()
    p_3.plot(y_3, 'r-')
    p_3.set_ylabel('normalized lift - negative', color='r')
