% Script to get statistics from user assessments. Uses pre-parsed sweep files saved on Google Drive.
%
% See also
% Assessment2sweeps
%
% N. Rajan and P. Silveira, May 2015
% BSX Proprietary
...
REPROCESS = 0;  % flag used to indicated whether to re-process existing process files.
...
SAVE_RESULTS = [USERPATH '\Google Drive\Tech_RD\Sensor\Research\LT Assessments\Assessment_Analysis_results']; % path and start of file name were results are saved at the end of analysis
...
[currents current256] = getCurrents; % get all currents indexed by current codes (Amps)
...
num_tot_assessments = length(assessmentList);   % total number of assessments to be processed (skip . and ..)
...
sweepLen = length(SWEEP_SUFFIX);
for ii = 1:num_tot_assessments
    fprintf('Processing assessment %d/%d: ', ii, num_tot_assessments);
    processFile = [SWEEP_DIR assessmentList(ii).name(1:end-sweepLen) PROCESS_SUFFIX];
    if ~REPROCESS && exist(processFile, 'file')
        fprintf('Skipping processing of existing file %s\n', processFile)
        continue
    end
    ...
    try     % don't stop because of a bug
        ...
        %     process.tHb = avgfiltNaN(calc_tHb(HbF), TISSUE_FILTER_LENGTH*sweep.samp_rate);    % calibrated total hemoglobin concentration (g/dL)
        %    [process.tissueTF, process.tissue] = istissue(HbF,HbConc);     % optical tissue detection
        
        process = sweep2process(sweep, device);
    ...
    mu_eff_Start(:,jj)  = trimmean(process.mu_eff(ST_RANGE,:), PERCENTILE, 1); % store one mu_eff value for every user
    mu_eff_End(:,jj) = trimmean(process.mu_eff(END_RANGE,:), PERCENTILE, 1);
    ...
    ccode15End(:,jj) = mode(sweep.ccode15(END_RANGE,:));
    ccode27End(:,jj) = mode(sweep.ccode27(END_RANGE,:));
    pH2OStart(:,jj) = trimmean(process.pH2O(ST_RANGE,:), PERCENTILE);
    pH2OEnd(:,jj) = trimmean(process.pH2O(END_RANGE,:), PERCENTILE);
    HbFStart(:,jj) = trimmean(process.HbF(ST_RANGE,:), PERCENTILE);
    HbFEnd(:,jj) = trimmean(process.HbF(END_RANGE,:), PERCENTILE);
    HbConcStart(:,jj) = trimmean(process.HbConc(ST_RANGE,:), PERCENTILE);
    HbConcEnd(:,jj) = trimmean(process.HbConc(END_RANGE,:), PERCENTILE);
    tHbStart(:,jj) = trimmean(process.tHb(ST_RANGE,:), PERCENTILE);
    tHbEnd(:,jj) = trimmean(process.tHb(END_RANGE,:), PERCENTILE);
    ...
    stat.tHb(jj) = baselineStats(process.tHb, BASELINE_RANGE*sweep.samp_rate);
    for kk = 1:numel(leds)
        stat.MuEff(jj,kk) = baselineStats(process.mu_eff(:,kk), BASELINE_RANGE*sweep.samp_rate);
        stat.OD15(jj,kk) = baselineStats(process.OD15(:,kk), BASELINE_RANGE*sweep.samp_rate);
        stat.OD27(jj,kk) = baselineStats(process.OD27(:,kk), BASELINE_RANGE*sweep.samp_rate);
    end
    ...
    % Save data
    save(processFile, 'process', 'stat');   % save process and stat structures
    save([WARMUP_DIR assessment.alpha__id WARMUP_SUFFIX], 'warmup');
    
    if PPT_FLAG     % save plots to PowerPoint
        generateAssessmentPlot(sweep, process);%.SmO2, OD15, OD27, process.HbF, process.pH2O, process.HbConc)
        if isfield(assessment, 'completed_on')
            ...
            saveppt2('ppt', ppt, 'fig', 1, 'stretch', 'off', 'textbox', ['Assessment ' assessment_id{jj} ' from user ' user_id{jj}])
        end
    end
    if isfield(assessment, 'completed_on')
        fprintf('Successfully processed assessment #%d from user %s performed on %s.\n', jj, assessment.user_id, assessment.completed_on)
    else
        fprintf('Successfully processed assessment #%d from user %s\n', jj, assessment.user_id)
    end
...
mu_s = calc_mu_s(wavel)/10;     % convert from 1/cm to 1/mm
...
% Find min and max tHb, HbF, pH2O and HbConc using fences
...
[min_tHb, max_tHb] = fences(tHbStart);
...
fprintf('tHb: \tLower inner fence = %5E \tUpper inner fence = %5E\n', min_tHb, max_tHb)
...
figure % Plot currents
set(gcf, 'Position', [807 -7 560 728]);
subplot(2,1,1)
boxplot((current256(ccode15End+1)*1000)',leds)
xlabel('LED centroid wavelengths (nm)', 'FontSize', FONTSIZE)
ylabel('Currents (mA)', 'FontSize', FONTSIZE)
title('15mm LEDs 15min into assessment', 'FontSize', FONTSIZE)
grid
subplot(2,1,2)
boxplot((current256(ccode27End+1)*1000)',leds)
xlabel('LED centroid wavelengths (nm)', 'FontSize', FONTSIZE)
ylabel('Currents (mA)', 'FontSize', FONTSIZE)
title('27mm LEDs 15min into assessment', 'FontSize', FONTSIZE)
grid

figure % Plot current codes
set(gcf, 'Position', [807 -7 560 728]);
subplot(2,1,1)
boxplot(ccode15End',leds)
xlabel('LED centroid wavelengths (nm)', 'FontSize', FONTSIZE)
ylabel('Current codes', 'FontSize', FONTSIZE)
title('15mm LEDs 15min into assessment', 'FontSize', FONTSIZE)
grid
subplot(2,1,2)
boxplot(ccode27End',leds)
xlabel('LED centroid wavelengths (nm)', 'FontSize', FONTSIZE)
ylabel('Current codes', 'FontSize', FONTSIZE)
title('27mm LEDs 15min into assessment', 'FontSize', FONTSIZE)
grid
