...
switch (LED_list(ii))
    ...
    case 7
        ...
        for i = 1:no_spectra,
            ...
            xi_sel = [x1(i) x2(i) x3(i) x4(i) x5(i) x6(i) x7(i)];
            W_pred(i) = -229 + [5.6 -113.8 315.5 -180]*xi_sel(LED_list)';  
            W_actual(i) = PC.WATER;
        end
        ...
        c(bv_index,:) = polyfit(W_actual,W_pred,2);
      
            coeff_matrix(bv_index,:) = b;
...
for bv_index = 1:31,
PC.BLOOD = 0.5+ 0.5*(bv_index-1);
water_pred(:,bv_index) = polyval(c(bv_index,:),water_pct);
subplot(221)
plot(water_pct,water_pred(:,bv_index))
hold on
subplot(222)
plot(water_pct,water_pred(:,bv_index)-water_pct')
hold on
end
subplot(221)
hold off
...
figure(2)
title('Dependence of 4-\lambda regression coeffs on % Bld vol','FontSize', 12)
pct_bldvol = 0.5+(0:29)*0.5;

p_b0 = polyfit(pct_bldvol,coeff_matrix(:,1)',5);
p_b1 = polyfit(pct_bldvol,coeff_matrix(:,2)',5);
p_b2 = polyfit(pct_bldvol,coeff_matrix(:,3)',5);
p_b3 = polyfit(pct_bldvol,coeff_matrix(:,4)',5);
p_b4 = polyfit(pct_bldvol,coeff_matrix(:,5)',5);

bo_fit = polyval(p_b0,pct_bldvol);
b1_fit = polyval(p_b1,pct_bldvol);
b2_fit = polyval(p_b2,pct_bldvol);
b3_fit = polyval(p_b3,pct_bldvol);
b4_fit = polyval(p_b4,pct_bldvol);

%save('C:\Documents and Settings\Owner\Desktop\Consulting\BSX athletics\Hydration data\polyfit_coeff.mat','p_b0','p_b1','p_b2','p_b3','p_b4');

plot(pct_bldvol,coeff_matrix,'*')
legend('b0','b1','b2','b3','b4')
text(6,-2250,'Lines are 5th-order poly fits')
hold on
plot(pct_bldvol,bo_fit,pct_bldvol,b1_fit,pct_bldvol,b2_fit,...
    pct_bldvol,b3_fit,pct_bldvol,b4_fit)
xlabel('Tissue blood volume (%)','FontSize', 12)
ylabel('Regression Coeff Magnitude','FontSize', 12)
hold off
