...
% LED sources with normalization by mean of all ODs
...
switch (LED_list(ii))
    ...
    case 7
        ...
        for i = 1:no_spectra,
            ...
            %mu_s = (mean_scat_coeff_at_800+std_scat_coeff_at_800*randn)*(wl/800).^(-0.22);
            ...
            %xn(i) = -(log10(sum(spectrum(1,:)'.*Rr(:,i)))+log10(sum(spectrum(2,:)'.*Rr(:,i)))+...
            %log10(sum(spectrum(3,:)'.*Rr(:,i)))+log10(sum(spectrum(4,:)'.*Rr(:,i))))/4;
            %xn(i) = -log10(sum(spectrum(2,:)'.*Rr(:,i)));
            xxn(1) = -log10(sum(spectrum(1,:)'.*Rr(:,i))); %Weighted OD at 665
            xxn(2) = -log10(sum(spectrum(2,:)'.*Rr(:,i))); %Weighted OD at 810
            xxn(3) = -log10(sum(spectrum(3,:)'.*Rr(:,i))); %Weighted OD at 850
            xxn(4) = -log10(sum(spectrum(4,:)'.*Rr(:,i))); %Weighted OD at 950
            xxn(5) = -log10(sum(spectrum970'.*Rr(:,i))); %Weighted OD at 970
            xxn(6) = -log10(sum(spectrum1020'.*Rr(:,i))); %Weighted OD at 1020
            xxn(7) = -log10(sum(spectrum1050'.*Rr(:,i))); %Weighted OD at 1050
            nrmfctr = mean(xxn(LED_list));
            x1(i) = xxn(1)/nrmfctr; x2(i) = xxn(2)/nrmfctr; x3(i) = xxn(3)/nrmfctr;
            x4(i) = xxn(4)/nrmfctr; x5(i) = xxn(5)/nrmfctr; x6(i) = xxn(6)/nrmfctr;
            x7(i) = xxn(7)/nrmfctr;
            
        end
        Xi = [x1' x2' x3' x4' x5' x6' x7'];  %Array of OD values for different LEDs
        Xi_sub = Xi(:,LED_list);
        %xx1 = Xi_sub(:,1)./(Xi_sub(:,3)-Xi_sub(:,4));
        %xx2 = Xi_sub(:,2)./(Xi_sub(:,3)-Xi_sub(:,4));
        %xx3 = Xi_sub(:,3)./(Xi_sub(:,3)-Xi_sub(:,4));
        %XXi = [xx1 xx2 xx3];
        ...
        rms_error = mean(abs(y_pred'-y));
            ...
            best_LEDindx4 = LEDindx4; best_y_pred = y_pred;
            best_y = y;best_b = b;
            end
    end
    end
...
switch (LED_list(ii))
    ...
    switch no_LEDs
        case 1
    text(50,30,['W= ' num2str(b(1),'%4.1f') '+' num2str(b(2),'%4.1f') ' OD_\lambda_1'])
        case 2
    text(50,30,['W= ' num2str(b(1),'%4.1f') '+' num2str(b(2),'%4.1f') ' OD_\lambda_1 +' num2str(b(3),'%4.1f') ' OD_\lambda_2'])
        case 3
    text(50,30,['W= ' num2str(b(1),'%4.1f') '+' num2str(b(2),'%4.1f') ' OD_\lambda_1 +' num2str(b(3),'%4.1f') ' OD_\lambda_2 +'  num2str(b(4),'%4.1f') ' OD_\lambda_3'])
        case 4
    text(50,30,['W= ' num2str(b(1),'%4.1f') '+' num2str(b(2),'%4.1f') ' OD_\lambda_1 +' num2str(b(3),'%4.1f') ' OD_\lambda_2 +'  num2str(b(4),'%4.1f') ' OD_\lambda_3+'  num2str(b(5,1),'%4.1f') ' OD_\lambda_4'])     
        case 5
    text(50,30,['W= ' num2str(b(1),'%4.1f') '+' num2str(b(2,1),'%4.1f') ' OD_\lambda_1 +' num2str(b(3),'%4.1f') ' OD_\lambda_2 +'  num2str(b(4),'%4.1f') ' OD_\lambda_3+'  num2str(b(5,1),'%4.1f') ' OD_\lambda_4+' num2str(b(6,1),'%4.1f') ' OD_\lambda_5'] )   
    end
