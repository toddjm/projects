% Script used to perform 3-wavelength multiple regression to find 
...
numreply = input('Enter desired 3rd wavelength in nm: ');
if isempty(numreply)
    Wave3 = 1020; %Default number is 100
else
    Wave3 = numreply; %
end
...
std_pct_SmO2 = 0;  %Estimated physiol std is 8%

mean_fract_dry_lipid = 0.28; %Estimated physiol mean is 0.28
std_fract_dry_lipid = 0.0; %Estimated physiol mean is 0.05
...
Wave3_index = (Wave3-WL_START)/WL_STEP+1;
...
x3(i) = -log10(Rr(Wave3_index,i)); %OD at 3rd wavelength
end
...
text(55,72,['\lambda_3= ' num2str(Wave3,'%3d') 'nm'])
text(50,30,...
    ['W= ' num2str(b(2,1),'%4.2e') ' OD_\lambda_1+' num2str(b(3,1),'%4.2e') ' OD_\lambda_2+' ' OD_\lambda_3+' num2str(b(4,1),'%4.2e')])
xlabel('Actual % water')
