...
DEV_RAD = 37/2;    % device radius (mm)
FABRIC_LENGTH = 60; % radius of extent of light-blocking fabric
LED_CURRENT = 113;  % leds current, mA
LED_CURRENT_NOM = 50;   % led nominal current (mA) - used during LED characterization
PD_LED_DIST = 27;   % max. photodiode-to-LEDs distance (mm)
% Tissue modeling
...
WL_POINTS = 600;
RHO_START = 2.5;  % shortest distance of interest (usually set to PD radius)
RHO_END = 80;   % longest distance of interest (usually set to maximum radius of limb of interest)
RHO_POINTS = 500;
...
OD = ones(RHO_POINTS,WL_POINTS)*NEOPRENE_THICKNESS/NEOPRENE_THICKNESS_NOM*3.66+0.88;
...
solar_flux_bare =  Rr.*repmat(solar_Irr.*PDA.*plastic, RHO_POINTS,1); % (A/cm^2nm)
i_photo_sun = 7*pi/4*trapz(rho,rho'.*trapz(wl,solar_flux,2)*1e-2);    % Amps (multiplied by 1E-2 to convert from 1/cm^2 to 1/mm^2 before integrating)
...
[RrLEDs, mu_eff] = getRr2(mu_a.total/10, mu_s/10, PD_LED_DIST, n);  % tissue transmission @ PD_LED_DISTmm, all wavelengths. Also mu_eff in 1/mm (doesn't change with distance)
...
figure
contour(wl, rho, 1e12*solar_flux_bare, 'ShowText', 'on');
ylabel('Distance (mm)')
xlabel('Wavelength (nm)')
%zlabel('I_{Sun} [nA/(cm^{2}nm)]')
%view(135,30)
