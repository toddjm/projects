function CalDOEAnalysis();

% Script used to analyze data collected during calibration DOE
%
% P. Silveira, Feb. 2015
%
...
DATE_START = '2015-02-17 00:00:00'; % start date to include in query
DATE_END = '2015-02-17 23:59:59';   % end date to include in query
...
ind1 = find(cradle == 2);
ind2 = find(cradle == 5);
[cradleFig, MaxAllCntCradleStat, MuEffCradleStat, RMSErrorCradleStat] = CalcStats(ind1, ind2, MaxAllCnt, MuEff, RMSError);

% Sort by box
ind1 = find(box == 2);
ind2 = find(box == 4);
[boxFig, MaxAllCntBoxStat, MuEffBoxStat, RMSErrorBoxStat] = CalcStats(ind1, ind2, MaxAllCnt, MuEff, RMSError);

% Sort by block
ind1 = find(block == 2);
ind2 = find(block == 3);
[blockFig, MaxAllCntBlockStat, MuEffBlockStat, RMSErrorBlockStat] = CalcStats(ind1, ind2, MaxAllCnt, MuEff, RMSError);

 %% Display results
 
%  FineTunePlots([MaxAllCntCradleStat.fig MuEffCradleStat.fig RMSErrorCradleStat.fig], 'Cradle 2', 'Cradle 5', MAX_PASS, MIN_PASS, MuEff)
%  FineTunePlots([MaxAllCntBoxStat.fig MuEffBoxStat.fig RMSErrorBoxStat.fig], 'Box 2', 'Box 4', MAX_PASS, MIN_PASS, MuEff)
%  FineTunePlots([MaxAllCntBlockStat.fig MuEffBlockStat.fig RMSErrorBlockStat.fig], '432-002', '432-003', MAX_PASS, MIN_PASS, MuEff)

 FineTunePlots(cradleFig, 'Cradle 2', 'Cradle 5', MAX_PASS, MIN_PASS, MuEff)
 FineTunePlots(boxFig, 'Box 2', 'Box 4', MAX_PASS, MIN_PASS, MuEff)
 FineTunePlots(blockFig, '432-002', '432-003', MAX_PASS, MIN_PASS, MuEff)
